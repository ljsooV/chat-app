...
void chat_window::update_status(const QString& text)
{
    ...
    m_ui->status_label->setStyleSheet(text == "Connected"
...
bool chat_window::process_buffered_packet()
{
    ...
    handle_packet(result.m_type,
                  QString::fromUtf8(result.m_payload.data(), static_cast<int>(result.m_payload.size())));
